...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	dbStartCmd.Flag("ad-realm", "(Only for SQL Server) Kerberos realm.").StringVar(&ccf.DatabaseADRealm)
	dbStartCmd.Flag("ad-spn", "(Only for SQL Server) Service Principal Name for Kerberos auth.").StringVar(&ccf.DatabaseADSPN)
