...
func TestListResources(t *testing.T) {
	...
	m := &mockedResourceAPIGetter{}
	m.mockListResources = func(ctx context.Context, req proto.ListResourcesRequest) (resources []types.ResourceWithLabels, nextKey string, err error) {
		return nil, "", nil
	}
	_, _, err := listResources(m, &http.Request{}, "unsupported")
	...
	testCases := []struct {
		...
		},
		{
			...
			expected: &proto.ListResourcesRequest{
				...
				SortBy:              &types.SortBy{Field: "foo", Dir: types.SortDir_SORT_DIR_DESC},
			},
		},
		{
		...
		},
		{
			...
			expected: &proto.ListResourcesRequest{
				...
				Limit:          defaults.MaxIterationLimit,
				SearchKeywords: []string{},
				SortBy:         &types.SortBy{Field: "foo", Dir: types.SortDir_SORT_DIR_UNSPECIFIED},
			},
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			m.mockListResources = func(ctx context.Context, req proto.ListResourcesRequest) (resources []types.ResourceWithLabels, nextKey string, err error) {
				if !tc.wantBadParamErr {
					...
					switch tc.expected.SortBy {
					case nil:
						require.Empty(t, req.SortBy)
					default:
						require.Equal(t, tc.expected.SortBy.Field, req.SortBy.Field)
						require.Equal(t, tc.expected.SortBy.Dir, req.SortBy.Dir)
					}
				}
				return nil, "", nil
			}

			_, _, err = listResources(m, req, types.KindNode)
...
type mockedResourceAPIGetter struct {
	...
	mockListResources         func(ctx context.Context, req proto.ListResourcesRequest) (resources []types.ResourceWithLabels, nextKey string, err error)
}
...
func (m *mockedResourceAPIGetter) ListResources(ctx context.Context, req proto.ListResourcesRequest) (resources []types.ResourceWithLabels, nextKey string, err error) {
