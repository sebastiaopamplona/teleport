...
You may create such roles after upgrading, but in the event of a cluster
...
>
The preset `auditor` role allows access to all events, sessions, and session
recordings. In order for the following roles to take effect you will need to
ensure your user doesn't also have a more permissive role like `auditor`.
</Admonition>
