...
- Set up access to your SQL Server using Active Directory authentication.
...
## Step 3/7. Join Linux node to Active Directory
...
AD use nameservers provided under networking details on the directory's overview
page.
...
<Admonition type="note" title="SPN">
  ...
  You can check if the user has any SPNs assigned by running `setspn -L alice`
  command on the Windows machine joined to your Active Directory domain and
  assign one using the `setspn -s user/alice alice` command.
</Admonition>
...
KVNO Principal
---- --------------------------------------------------------------------------
   5 alice@DBADIR.TELEPORTDEMO.NET (aes256-cts-hmac-sha1-96)
   2 bob@DBADIR.TELEPORTDEMO.NET (aes256-cts-hmac-sha1-96)
```
...
| `--ad-realm` | Kerberos realm (Active Directory domain) that SQL Server is joined. |
...
You can use `adutil spn search` command to see the SPNs registered for your SQL
Server. Typically, they take a form of `MSSQLSvc/<name>.<ad-domain>:<port>`.

For example, AWS RDS SQL Server named `sqlserver` and joined to an AWS managed
Active Directory domain `EXAMPLE.COM` will have the following SPNs registered:

```bash
$ adutil spn search
...
CN=EC2AMAZ-4KN05DU,OU=RDS,OU=AWS Reserved,DC=example,DC=com:
    MSSQLSvc/sqlserver.example.com:1433
    MSSQLSvc/EC2AMAZ-4KN05DU.example.com:1433
    MSSQLSvc/EC2AMAZ-4KN05DU.example.com
...
```

Alternatively, you can look SPNs up in the Attribute Editor of Active Directory
Users and Computers dialog on your AD joined Windows machine. RDS SQL Server
object typically resides under AWS Reserved / RDS path:
...
- [Manually join a Linux instance](https://docs.aws.amazon.com/directoryservice/latest/admin-guide/join_linux_instance.html) in AWS documentation.
- [Introduction to `adutil`](https://docs.microsoft.com/en-us/sql/linux/sql-server-linux-ad-auth-adutil-introduction) in Microsoft documentation.
