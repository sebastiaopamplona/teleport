...
<Details title="Teleport Cloud Warning" scopeOnly={true} scope={["cloud"]}>
Since it is not currently possible to make AWS credentials available to an Auth Service running on Teleport Cloud, Teleport Cloud does not support the EC2 join method.
</Details>
